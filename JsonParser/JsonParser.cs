	...
	public static class JsonParser {
		...
		static JsonParser() {
			...
			json_number_parser = Sequence([
					new () { ParserDirect = Char('-', "number_negative_sign"), Optional = true, Include = true },
				new () { ParserDirect = Any_of(['1','2','3','4','5','6','7','8','9'], "number_first_digit"), Optional = false, Include = true },
				new() { ParserDirect = Many(Digit(), "number_after_first_digits").Map(x => string.Concat(x)), Optional = true, Include = true },
				new() {
					ParserDirect = Sequence([
						new() { ParserDirect = Char('.', "number_fraction_delimiter"), Optional = false, Include = true },
						new() { ParserDirect = Many(Digit(), "number_fraction_digits").Map(x => string.Concat(x)), Optional = false, Include = true },
				...
				new() {
					ParserDirect = Sequence([
						new() { ParserDirect = Any_of(['e', 'E'], "number_exponent_delimiter"), Optional = false, Include = true },
						new() { ParserDirect = Any_of(['+', '-'], "number_exponent_sign"), Optional = true, Include = true },
						new() { ParserDirect = Many(Digit(), "number_exponent_digits").Map(x => string.Concat(x)), Optional = false, Include = true },
			...
			json_character_parser = Any([
				...
				Sequence([
					new() { ParserDirect = Char('\\', "escaped_start"), Optional = false, Include = true },
					new() { ParserDirect = Any([
						...
						Sequence([
							new() { ParserDirect = Char('u', "escaped_unicode_start"), Optional = false, Include = true },
							new() { ParserDirect = Many(Satisfies(c => char.IsAsciiHexDigit(c), "escaped_unicode_hex_digit").Expect("Expected Hex-Digit"), "escaped_unicode_hex", 4, 4).Expect("Expected exactly 4 Hex-Digits"), Optional = false, Include = true },
			...
			json_string_parser = Sequence([
				new() { ParserDirect = Char('"', "string_start"), Optional = false, Include = false },
				new() { ParserFunc = () => Many(json_character_parser, "object_member_identifier").Map(x => string.Concat(x)), Optional = false, Include = true },
				new() { ParserDirect = Char('"', "string_end").Expect($"Expeceted '\"'"), Optional = false, Include = false },
			], "string").Map(x => x[0]);

			json_object_member_parser = Sequence([
				new() { ParserDirect = Whitespace(), Optional = true, Include = false },
				new() { ParserFunc = () => json_string_parser.Expect("Expected Key"), Optional = false, Include = true,
				},
				new() { ParserDirect = Whitespace(), Optional = true, Include = false },
				new() { ParserDirect = Char(':', "object_member_delimiter").Expect($"Expeceted ':'"), Optional = false, Include = false },
			...
			json_object_members_parser = Sequence([
				...
				new() { ParserDirect = Many(Sequence([
					new() { ParserDirect = Char(',', "object_members_delimiter"), Optional = false, Include = false },
					new() { ParserFunc = () => json_object_member_parser, Optional = false, Include = true },
			...
			json_object_parser = Sequence([
				new() { ParserDirect = Char('{', "object_start"), Optional = false, Include = false },
				new() { ParserFunc = () => json_object_members_parser, Optional = true, Include = true },
				new() { ParserDirect = Whitespace(), Optional = true, Include = false },
				new() { ParserDirect = Char('}', "object_end").Expect($"Expeceted '}}'"), Optional = false, Include = false },
			], "object").Map(x => {
				if (x.Count > 0) {
			...
			json_array_parser = Sequence([
				new() { ParserDirect = Char('[', "array_start"), Optional = false, Include = false },
				new() { ParserFunc = () => json_elements_parser, Optional = true, Include = true },
				new() { ParserDirect = Whitespace(), Optional = true, Include = false },
				new() { ParserDirect = Char(']', "array_end").Expect($"Expeceted ']'"), Optional = false, Include = false },
			], "array").Map(x => {
				if (x.Count > 0) {
			...
			json_elements_parser = Sequence([
				new() { ParserFunc = () => json_element_parser, Optional = false, Include = true },
				new() { ParserDirect = Many(Sequence([
					new() { ParserDirect = Char(',', "elements_delimiter"), Optional = false, Include = false },
					new() { ParserFunc = () => json_element_parser, Optional = false, Include = true },
